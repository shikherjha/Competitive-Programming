...
void solve() {
    ...
    vi pref_odd(n,0), pref_even(n,0);
    pref_odd[0] = a[0];
    //pref_even[0] = a[1];
    f(i,1,n){
        pref_odd[i] = pref_odd[i-1] + ((i%2==0)? a[i]:0);
        pref_even[i] = pref_even[i-1] + ((i%2)? a[i]:0);
    }
    //_print(pref_odd);
   // _print(pref_even);
    set<int> seen;
    f(i,0,n){
        int diff = pref_odd[i] - pref_even[i];
        //cout << diff << endl;
        if(seen.count(diff) || diff==0){
            cout << "YES" << endl;
            return;
        }
        seen.insert(diff);
