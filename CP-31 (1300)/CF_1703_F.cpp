...
void solve() {
    ...
    for(auto& it:tmp){
        if(it.first<it.second) {
            pref[idx] = 1;
            tmp2.push_back({it.first,it.second});
        }
        idx++;
    }
   ...
   for(auto& it:tmp2){
        ...
        //cout << val << ' ' << idx << endl;
        auto it1 = upper_bound(all(tmp2),make_pair(idx,INT_MAX),[](auto& a, auto& b){
            return a.first<b.first;
        });
        if(it1!=tmp2.end()){
            //cout << it1->first << ' ' << it1->second << endl;
            if(flag[it1->second-1]){
                //cout << pref[n-1] << ' ' << pref[it1->second-2]<< endl;
                int i = mp[it1->first];
                ans += pref[n-1] - ((i-1>=0)? pref[i-1]:0);
            }
            //cout << ans << endl << endl;
        }
   } 
