...
void solve() {
    ...
    f(i,0,k){
        ...
        if(curr>0){
            while(!right.empty() && abs(curr)>=(*right.begin()).first){
                auto i = (*right.begin()).second;
                ans--;
                right.erase(right.begin());
                left.erase(left.find({tmp[i].first,i}));
            }
        }
        else{
            while(!left.empty() && abs(curr)>=(*left.begin()).first){
                auto i = (*left.begin()).second;
                ans--;
                left.erase(left.begin());
                right.erase(right.find({tmp[i].second,i}));
            }
        }
        cout << ans << ' ';
    }
