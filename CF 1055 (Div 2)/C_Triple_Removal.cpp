...
void solve() {
   ...
   vi pre(n),cnt0(n),cnt1(n);
   pre[0] = 0; cnt0[0] = (a[0]==0)? 1:0; cnt1[0] = (a[0]==1)? 1:0;
   for(int i=1;i<n;i++){
        pre[i] = pre[i-1] + ((a[i-1]==a[i])? 1:0);
        cnt0[i] = cnt0[i-1] + ((a[i]==0)? 1:0);
        cnt1[i] = cnt1[i-1] + ((a[i]==1)? 1:0);
   }
   //_print(cnt0);
   while(q--){
        int l,r; cin >> l >> r;
        l--; r--;
        int len = r-l+1;
        if (len%3!=0) {cout << -1 << endl; continue;}

        int cnt_0 = cnt0[r] - ((l>0)? cnt0[l-1]:0);
        int cnt_1 = cnt1[r] - ((l>0)? cnt1[l-1]:0);
        if(cnt_0%3!=0 || cnt_1%3!=0){
