...
void solve() {
    ...
    // dp[i][j] -> represents maximum height from index i to n-1, given the ith index if from jth row (0->first, 1->second)
    vvi dp(n+1,vi(2,0));
    int maxi;
    rf(i,n-1,0){
        f(j,0,2){
            if(j==0){
                maxi = max(0+dp[i+1][j], a[i]+dp[i+1][1]);
            }
            else{
                maxi = max(0+dp[i+1][j],b[i]+dp[i+1][0]);
            }
            dp[i][j] = maxi;
        }
    }
