...
void solve() {
    ...
    f(i,1,n+1){
        if(a[i]>b[i]){
            cout << "Case #" << val++ << ": " << -1 << endl;
            return;
        }
    }
    vvi tmp1(maxi+1), tmp2(maxi+1);
    f(i,1,n+1){
        tmp1[a[i]].push_back(i);
        if(a[i]!=b[i]) tmp2[b[i]].push_back(i);
    }

    
    vpi ans;
    for(int i=1; i<=maxi; i++){
        if(tmp2[i].empty()) continue;
        if(tmp1[i].empty()){
            cout << "Case #" << val++ << ": " << -1 << endl;
            return;
    ...
    if(ans.empty()){
        cout << "Case #" << val++ << ": " << 0 << endl;
    }
    else{
        cout << "Case #" << val++ << ": " << sz(ans) << endl;
        for(auto& it:ans){
