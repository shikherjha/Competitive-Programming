...
void solve() {
    ...
    // dp[i][j] -> represents minimum ops to cut squares from a rectangle of dimension ixj
    vvi dp(a+1,vi(b+1,INF));

    // Base case
    f(i,1,a+1){
        f(j,1,b+1){
            if(i==j) dp[i][j] = 0; // when both dimension are equal we don't need use more operations
        }
    }
    // Transitions
        // for(j->1,a-1) dp[i][j] = min(dp[i])
