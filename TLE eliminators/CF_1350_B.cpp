...
void solve() {
    ...
    vvi factors = precompute(n);

    //dp[i] -> represents the maximum length possible using the ith index as the last 
    vi dp(n,1);

    for(int i=1;i<n;i++){
        for(auto& x:factors[i+1]){
            if(x==i+1) continue;
            //cout << i+1<< ' ' << x << endl;
            //cout << a[i] << ' ' << a[x-1] << endl;
            if(a[x-1]<a[i]) dp[i] = max(dp[i], dp[x-1] + 1);
