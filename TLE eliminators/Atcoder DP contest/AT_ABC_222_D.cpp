...
void solve() {
    ...
    int len = maxi - mini + 1;
    // dp[i][j] -> represents the no. of ways to assign upto to ith index
    vi prev(len,0),curr(len,0);
    f(i,a[0],b[0]+1){
        prev[i-mini] = 1; // base case
    }
    f(i,1,n){
        vi prefix_sum(len,0);
        int tmp = 0;
        f(x,0,len){
            tmp = mod_add(tmp,prev[x]);
            prefix_sum[x] = tmp;
        }
        f(j,0,len){
            int val = j + mini;
            if(val<a[i] || val>b[i]) curr[j] = 0;
            else curr[j] = prefix_sum[j];
            
        }
        prev.swap(curr);
        fill(all(curr),0);
    }
    cout << accumulate(all(prev),0LL)%MOD << endl;
