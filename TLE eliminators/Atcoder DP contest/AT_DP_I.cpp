...
void solve() {
    ...
    f(i,0,n) cin >> a[i];
    int min_head = (n+1)/2;
    // dp[i][j] -> represents total probability of getting more head till index i
    vector<vector<double>> dp(n,vector<double>(n+1,0.0));
    // Base case
    dp[0][0] = 1-a[0];
    dp[0][1] = a[0];
    // Transitions
        // dp[i][j] = (a[i]*dp[i-1][j-1] + (1-a[i])*dp[i-1][j]) picking head or tail at the ith index
    
    for(int i=1;i<n;i++){
        for(int j=0;j<=n;j++){
            if(j-1>=0) dp[i][j] += a[i]*dp[i-1][j-1];
            dp[i][j] += (1-a[i])*dp[i-1][j];
        }
    }
    double ans = 0.0;
    for(int j=min_head;j<=n;j++){
        ans += dp[n-1][j];
    }
