...
void solve() {
	...
	int cnt = 0; // count of elements lesser than or equal to k
	vi idx;
	f(i,0,n){
		if(a[i]<=k){
			cnt ++;
	...
	// case when count==2
		...
		int idx1=idx[0],idx2=idx[1]; // indexes of both
		if(abs(idx1-idx2)>3){
			cout << "No" << endl;
		}
		else{
	...
	// condition for cnt==3
	    ...
	    for(int i = 0; i < 3; ++i) {
	        for(int j = i+1; j < 3; ++j) {
	            if(abs(idx[i] - idx[j]) <= 2) {
	                if(idx[i] == 0 || idx[j] == 0 || idx[i] == n-1 || idx[j] == n-1) {
	                    ok = true;
	                }
	            }
	        }
