...
void solve() {
    ...
    sort(all(b));
    vi pref(m + 1, 0);
    f(i, 0, m) pref[i + 1] = mod_add(pref[i], b[i]);

    int total_ans = 0;
    f(i, 0, n) {
        int k = lower_bound(all(b), a[i]) - b.begin();
        
        int term1 = mod_mul(k, a[i]);
        term1 = mod_sub(term1, pref[k]);

        int sum_greater_b = mod_sub(pref[m], pref[k]);
        int term2 = mod_sub(sum_greater_b, mod_mul(m - k, a[i]));

        total_ans = mod_add(total_ans, mod_add(term1, term2));
    }
    cout << total_ans << endl;
}
