...
void solve() {
    ...
    // dp[i][j] -> represents minimum cost from ith index with jth being the last index
    vi next(n+2,0);
    vi curr(n+1,INF);

    // Transitions
        // if(a[i]>a[j]) = min{c[i]+dp[i+1][j],dp[i+1][i]}
        // if(a[i]==a[j]) = dp[i+1][j]
        // if(a[i]<a[j]) = c[i] + dp[i+1][j]

    for(int i=n;i>=1;i--){
        for(int j=0; j<i; j++){
            if(a[i]>a[j]){
                curr[j] = min(c[i] + next[j], next[i]);
            }
            else if(a[i]==a[j]){
                curr[j] = next[j];
            }
