...
void solve() {
    ...
    vpi tmp;
    for(auto& it:mp){
        tmp.push_back({it.second*it.first,it.first});
    }
   ...
   //_print(tmp);
    int ans = 0;
    int prev = INF;
    int vertices = 0, odd_cnt = 0;
    for(auto& it:tmp){
        int cnt = it.first/it.second, side = it.second;
        //if(2*prev<=side) continue;
        if(cnt%2==0){
            ans += cnt*side;
            vertices += cnt;
        }
