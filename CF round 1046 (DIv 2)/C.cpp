...
void solve() {
    ...
    sort(iv.begin(), iv.end(), [](const array<int,3>& x, const array<int,3>& y){
    ...
    int m = (int)iv.size();
    vector<int> ends(m);
    for (int i = 0; i < m; ++i) ends[i] = iv[i][0];

    vector<int> dp(m+1, 0);
    for (int i = 1; i <= m; ++i) {
        int l = iv[i-1][1];
        int w = iv[i-1][2];
        int idx = (int)(upper_bound(ends.begin(), ends.begin() + (i-1), l-1) - ends.begin());
