...
void solve() {
    ...
    if(!flag){
        ...
        vi ans(n);
        sort(all(tmp));
        int last = tmp[n-1].second;
        for(int i=n-1;i>=1;i--){
            auto it1 = tmp[i], it2 = tmp[i-1];
            if(it1.first!=it2.first){
                ans[it1.second] = last + 1;
                last = it2.second;
            }
            else{
        ...
        for(int i=n-1;i>=1;i--){
            auto it = tmp[i];
            auto it1 = tmp[i-1];
            ans[it.second] = it1.second+1;
        }
        ans[tmp[0].second] = tmp[n-1].second+1;
        for(auto& x:ans){
            cout << x << ' ';
        }
        cout << endl;
