    ...
    f(len, 1, n + 1) {
        f(i, 0, n - len + 1) {
            ...
            if(len == 1) {
                ...
                dp_max[i][j] = -INF;
                f(k, i, j) {
                    int val_min = dp_min[i][k] + 2*dp_min[k + 1][j];
                    int val_max = dp_max[i][k] + 2*dp_max[k + 1][j];
                    if(val_min<dp_min[i][j]) dp_min[i][j]= val_min;
                    if(val_max>dp_max[i][j]) dp_max[i][j]=val_max;
                }
            }
        }
    }
