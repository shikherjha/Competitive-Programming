...
void solve() {
    ...
    f(l,0,n){
        int needA =  a+prefA[l];
        auto it1 = lower_bound(all(prefA),needA);
        if(it1==prefA.end()) continue;
        int r1 = it1-prefA.begin();

        int needB = b+prefB[l];
        auto it2 = lower_bound(all(prefB),needB);
        int r2 = ((it2==prefB.end())? n+1:it2-prefB.begin());
        r2 = min(r2,n+1);
        if (r2>r1) ans += (r2-r1);
    }
    cout << ans << endl;
